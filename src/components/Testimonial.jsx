...
const Testimonial = ({testimonials})=>{
  ...
  //Setting ttl to 15 minutes (15*60*1000)
    ...
    async function addTestimonial(data){
      ...
      console.log(data);
      if(data.testimonial.trim().length===0||data.testimonial.trim().length===0){
        ...
        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
          ...
          />
            ...
            <div className="flex items-center p-6 space-x-2 ">
              <button disabled={progress} type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 inline-flex items-center">
              {progress? <span><svg aria-hidden="true" role="status" class="inline w-4 h-4 mr-3 text-white animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    </svg>Saving...</span>:"Submit"}
</button>
                
                <button id="modalCloseButton" data-modal-toggle="defaultModal" type="button" className="text-white bg-red-600 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-red-800 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Close</button>
            </div>
            </form>
        </div>
    </div>
